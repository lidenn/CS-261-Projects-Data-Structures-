...
{
	assert(v!=NULL);
	TYPE * oldbuffer = v->data;
       	int oldsize = v->size;
	initDynArr(v, newCap);
	
	int i;
	for(i=0; i<oldsize; i++){
		v->data[i] = oldbuffer[i];
	}	
	v->size = oldsize;  /*size was set to 0 after init*/
	free(oldbuffer);
...
void addDynArr(DynArr *v, TYPE val)
{
	...
	if(v->size >= v->capacity){
		_dynArrSetCapacity(v, (v->size)*2); /*double size*/
	}
	v->data[v->size] = val; /*size is at the next index*/
	v->size++;
}
...
{
	assert(v!=NULL);
	while(idx<(v->size-1)){ /*-1 because if you do it to last index, you run out of range*/
