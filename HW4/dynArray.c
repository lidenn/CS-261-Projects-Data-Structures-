...
void addHeap(DynArr *heap, TYPE node)
{
	...
	while(pos >0 && compare(node, parentVal)==-1){
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
	...
	assert(heap);
	int left_index = pos*2+1;
	int right_index = pos*2+2;
	if(right_index < max){
		int small_index = _smallerIndexHeap(heap, left_index, right_index);	
		if( compare(heap->data[small_index], heap->data[pos])==-1){
			swapDynArr(heap, pos, small_index);
			_adjustHeap(heap, max, small_index); /*Continues until bottom (recursive)*/

		}
	}
	else if (left_index < max){
		if( compare(heap->data[left_index], heap->data[pos])==-1){
			swapDynArr(heap, pos, left_index);
			_adjustHeap(heap, max, left_index); /*Continues until bottom (recursive)*/
		}
	}
	else{
		return;
...
void removeMinHeap(DynArr *heap)
{
	...
	heap->size--; /*removes the last element and decrements size*/
	_adjustHeap(heap, last-0, 0); /*recursive function to percolate*/
...
{
	/* FIXME */
	int max_index = heap->size;
	int i;
	for(i=max_index/2-1; i >=0; i--){ /*you can start at the parents because leafs are already heaps*/ 
		_adjustHeap(heap, max_index, i);
